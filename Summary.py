...
def cal_time(data_old, data_new, deliver_Accept_Name):
    ...
    for i in data_old.index.values:
        ...
        if not data_new_Isin.empty:
            time_new_old = (data_new_Isin['Date']-data_old['Date'].iloc[i]
...
df = df.drop_duplicates(
    subset=['Transaction Type', 'Order Number', 'Line'], keep='last')
# 去掉空行
...
# 计算Receive到Accept时间
accept_Receive_Name = ['Item', 'Order Number', 'Line',
                       "Receive Date", "Accept Date", 'Receive to accept time']
...
timedf_Accept_Receive.to_excel(
    writer, sheet_name='Receive to accept', index=False)
timedf_Deliver_Accept.to_excel(
    writer, sheet_name='Accept to deliver', index=False)
writer.save()
